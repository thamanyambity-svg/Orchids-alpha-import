...
export default function AdminExchangeRatesPage() {
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-2">{t("admin.rates.title", "Taux de change")}</h1>
      ...
      <form
        onSubmit={publish}
        className="mb-10 grid gap-4 rounded-xl border border-white/10 bg-white/5 p-5 sm:grid-cols-[120px_160px_1fr_auto] sm:items-end"
      >
        <div className="space-y-2">
          <Label htmlFor="rate-currency" className="text-white/60">
        ...
        <div className="space-y-2">
          <Label htmlFor="rate-value" className="text-white/60">
        ...
        <div className="space-y-2">
          <Label htmlFor="rate-notes" className="text-white/60">
      ...
      {loading ? (
        ...
        <div className="space-y-10">
          <section>
            ...
            {active.length === 0 ? (
              <div className="p-12 text-center border-2 border-dashed border-white/10 rounded-2xl">
                ...
                <h3 className="font-semibold text-white/50">{t("admin.rates.empty", "Aucun taux publié")}</h3>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {active.map((rate, i) => (
                  ...
                  >
                    ...
                    {rate.notes && <p className="mt-2 text-xs text-white/40">{rate.notes}</p>}
                  </motion.div>
                ))}
              </div>
            )}
          ...
          <section>
            ...
            {rates.length === 0 ? (
              <p className="text-sm text-white/30">{t("admin.rates.no_history", "Rien à afficher.")}</p>
            ) : (
              <div className="space-y-2">
                {rates.map((rate) => (
                  <div
