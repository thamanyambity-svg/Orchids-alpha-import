...
async function resetAdminPassword() {
    ...
    if (!email || !newPassword || newPassword.length < 12) {
        console.error('❌ ADMIN_EMAIL et ADMIN_PASSWORD (≥ 12 caractères) requis. Aucune valeur en dur.')
        console.error('   Exemple: ADMIN_EMAIL="a@b.com" ADMIN_PASSWORD="MotDePasseFort" npx tsx scripts/reset_admin_password.ts')
        process.exit(1)
    }
