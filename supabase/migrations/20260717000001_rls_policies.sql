...
-- Migration: 20250308000001 (re-numbered to run after schema)
-- Description: Politiques de sécurité pour protéger les données par rôle
-- IMPORTANT: Exécuter APRÈS 20260717000000_complete_schema.sql
...
DROP POLICY IF EXISTS "payments_via_order" ON public.payments;
...
-- -----------------------------------------------------------------------------
-- INVOICES (lecture buyer/partner via order, admin tout)
-- -----------------------------------------------------------------------------
...
-- PROCESSED_STRIPE_EVENTS (table réservée au service role / webhook)
-- RLS activé sans policy = tout accès client refusé
ALTER TABLE public.processed_stripe_events ENABLE ROW LEVEL SECURITY;

-- INBOUND_EMAILS (Admin read/update, insert via webhook service role)
...
-- SEPA PAYMENT TRANSACTIONS (Admin read, service role write)
ALTER TABLE public.sepa_payment_transactions ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "sepa_payment_transactions_admin" ON public.sepa_payment_transactions;
CREATE POLICY "sepa_payment_transactions_admin" ON public.sepa_payment_transactions
  FOR SELECT USING (get_user_role() = 'ADMIN');

-- SEPA PAYMENT RETRIES (Admin read)
ALTER TABLE public.sepa_payment_retries ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "sepa_payment_retries_admin" ON public.sepa_payment_retries;
CREATE POLICY "sepa_payment_retries_admin" ON public.sepa_payment_retries
  FOR SELECT USING (get_user_role() = 'ADMIN');